...
int main(int argc, char *argv[])
{
    ...
    int t_max, i_max, error_condition;
    double time;

    /* Each MPI process must exactly once initialize the MPI runtime system
    before calling any other MPI routine. */
    error_condition = MPI_Init(&argc, &argv);
    if (error_condition != MPI_SUCCESS)
    {
        fprintf(stderr, "Unable to setup MPI runtime system.\n");
        /* Finalize and shut down all MPI processes referred to by
        the standard communicator and abort program execution. */
        MPI_Abort(MPI_COMM_WORLD, error_condition);
    }
    ...
    /* Each MPI process must exactly once finalize the MPI runtime system
    after calling any other MPI routine. */
    error_condition = MPI_Finalize();
