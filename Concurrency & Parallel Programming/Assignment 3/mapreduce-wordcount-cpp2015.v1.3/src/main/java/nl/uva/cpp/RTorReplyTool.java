...
public class RTorReplyTool extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        job.setMapperClass(RTorReplyMapper.class);
        job.setReducerClass(RTorReplyReducer.class);
        ...
        if (conf.get("mapreduce.framework.name").equals("yarn")) {
            job.addCacheFile(new URI("/projects/cpp2015/" + RTorReplyMapper.parseModelPath));
            job.addCacheFile(new URI("/projects/cpp2015/" + RTorReplyMapper.sentimentModelPath));
        }
