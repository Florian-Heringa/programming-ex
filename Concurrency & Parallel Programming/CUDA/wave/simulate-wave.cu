...
__global__ void simulateWaveKernel(float* deviceA, float* deviceB, float* deviceResult, int i_max) {
    ...
    // Fine-grained parallelization; each kernel will compute the amplitude
    // at only one location derived from its index.
...
void simulateWaveCuda(int i_max, int t_max, float* old, float* current, float* next) {
    ...
    // We will execute as many kernels as there are amplitude points,
    // except for the endpoints which we will leave to 0, for each timestep
    // and rotate the wave arrays around.
    ...
    // Copy the next wave after t_max - 1 steps or the current wave after t_max steps back.
    checkCudaCall(cudaMemcpy(current, deviceB, i_max * sizeof(float), cudaMemcpyDeviceToHost));
