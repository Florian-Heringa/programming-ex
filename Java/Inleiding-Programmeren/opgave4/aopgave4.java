...
public class aopgave4 {   
	...
	private static void readTxtToDatabase (String filename) {
		...
		// 	while (line != null) {
		// 		addStringToDatabase(line, false);
		// 	}
		// 	br.close();
		// } catch (IOException e) {
		...
		try {
			ReadFile file = new ReadFile(filename + ".txt");
			String[] allLines = file.openFile();

			for (String s : allLines) {
				addStringToDatabase(s, false);
			}		
		} catch (IOException e) {
			System.out.println(e.getMessage());
		}
	...
	public static void callUI() {
		...
		do {
			...
			if(command.equals("HELP")) {
				...
				readTxtToDatabase(stringArray[1]);
				System.out.printf("%s.txt loaded into database....\n\n", stringArray[1]);           
			}
				...
				try {
					if (stringArray[1].length() != 6 | stringArray[2].length() != 6) {
						System.out.println("string(s) of incorrect length....");
						System.out.println();
					} else {
...
class ReadFile {
	...
	public String[] openFile () throws IOException {
		...
		String[] txtData = new String[numLines];
